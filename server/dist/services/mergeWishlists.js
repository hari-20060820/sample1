...
const normalise = (item) => ({ ...item, source: Array.isArray(item.source) ? [...new Set(item.source)] : [] });
const sameItems = (a, b) => JSON.stringify(a) === JSON.stringify(b);
export function mergeWishlists(a, b, strategy = {}) {
    if (a.id === b.id && a.version === b.version && sameItems(a.items, b.items))
        return { ...a };
    const items = new Map();
    ...
    for (const raw of b.items) {
        ...
        if (!existing) {
            items.set(incoming.productId, incoming);
            continue;
        }
